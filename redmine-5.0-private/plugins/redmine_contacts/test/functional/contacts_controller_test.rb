...
class ContactsControllerTest < ActionController::TestCase
  ...
  def setup
    ...
    ContactsSetting[:show_closed_issues, nil] = '0'
  end
  ...
  def test_get_show_with_related_issue
    ...
    prepare_related_issues(issue, contact)
  ...
  def test_get_show_with_closed_issues
    @request.session[:user_id] = 1
    Setting.default_language = 'en'
    contact = Contact.find(3)
    issue = Issue.find(2)
    ...
    compatible_request :get, :show, id: contact.id
    assert_response :success

    assert_select 'div#contact_issues h3', :html => /Related issues/
    assert_select "tr#contact_issue_#{issue.id}", count: 0
  end
